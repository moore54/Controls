...
%     ptr_a+1 = 2;
    ...
    end_node   = [waypoints(1:4,ptr_a+1)', 0, 0];
...
switch state_transition,
    case 0% beginning of simulation
        flag   = 2;
        Va_d  = P.Va0;% waypoints(5,ptr_a);
%         Rmin = Va_d^2/P.gravity/tan(P.phi_max);
%         start_node = [waypoints(1:4,ptr_a)', 0, 0];
%         end_node   = [waypoints(1:4,ptr_a+1)', 0, 0];
%         dubinspath = dubinsParameters(start_node, end_node, Rmin);
        r  = waypoints(1:3,ptr_a+1);%[0; 0; 0];     % not used for orbit
        q  = dubinspath.q1;%[1; 0; 0];     % not used for orbit
        c      = [0;0;0];
        rho    = 0;
        lambda = 0;
    ...
    case 1% follow first orbit on Dubins path until intersect H1
        ...
        Va_d  = P.Va0;% waypoints(5,ptr_a); % desired airspeed along waypoint path
%         Rmin = Va_d^2/P.gravity/tan(P.phi_max);
%         start_node = [waypoints(1:4,ptr_a)', 0, 0];
%         end_node   = [waypoints(1:4,ptr_a+1)', 0, 0];
%         dubinspath = dubinsParameters(start_node, end_node, Rmin);
...
% testout = (p-dubinspath.w1)'*dubinspath.q1 
