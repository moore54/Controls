...
function path_out=planRRTDubins(wpp_start, wpp_end,Rmin, map)
...
path_d = dubanize(path);
path_out = smoothPath(path_d,map);
path_out_d = dubanize(path_out);
plotmap(map,path_d,path_out_d,tree,Rmin);

end

function path_new = dubanize(path)
path_new = path;
...
function plotmap(map,path,smoothedPath,tree,Rmin)
...
for i=2:size(tree,1),
    ...
    waypoints_old = [X',Y',Z',[0,0]'];
    waypoints = dubanize(flipud(waypoints_old));


    XX = [];
    YY = [];
    for i=2:size(waypoints,1),
        dubinspath = dubinsParameters(waypoints(i-1,:),waypoints(i,:),Rmin);
