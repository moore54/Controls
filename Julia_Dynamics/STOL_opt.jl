...
function obj(x,ds,h_set,dt,TW,optimize,P)
    ...
    C = [C_maxheight/h_set;C_vel/10;C_minheight;C_turnback/10]
    # println("3")
...
Mass_array = [5.0,10.0,15.0]
# args = []
args2 = []
optimize = []
errorsum = zeros(Mass_array)
mean_thrust2weight = zeros(Mass_array)
total_energy = zeros(Mass_array)
final_dist = zeros(Mass_array)
...
# i = 1
    ...
    if !testrun
        ...
        xopt, fopt, optinfo = snopt(objcon, x0, lb, ub, options;printfile = "$(fileLoc)/snopt-print$(Mass_array[i]).out", sumfile = "$(fileLoc)/snopt-summary.out") #!!!!!! ARGS are in objcon wrapper function !!!!!!!!#
    ...
    PyPlot.plot(savestates[:,1],CL,".-",label = "$(round.(Mass,4))")
    PyPlot.xlabel("pn (m)")
    PyPlot.ylabel("CL")
    legend(loc="center left", title = "Mass (kg)",bbox_to_anchor=(1, 0.5))
    PyPlot.savefig("./figures/changing_mass/$figname.png",transparent = true)

    figname = "d_q"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1],savestates[:,6],".-",label = "$(round.(Mass,4))")
    PyPlot.xlabel("pn (m)")
    PyPlot.ylabel("q")
    legend(loc="center left", title = "Mass (kg)",bbox_to_anchor=(1, 0.5))
    PyPlot.savefig("./figures/changing_mass/$figname.png",transparent = true)
    ...
    figname = "d_AOA"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1],alpha_save,".-",label = "$(round.(Mass,4))")
    PyPlot.xlabel("pn (m)")
    ...
    figname = "d_Va"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1],Va,".-",label = "$(round.(Mass,4))")
    PyPlot.xlabel("pn (m)")
    ...
    figname = "pn_pd"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1],-savestates[:,2],".-",label = "$(round.(Mass,4))")
    PyPlot.xlabel("pn (m)")
    PyPlot.ylabel("-pd (m)")
    legend(loc="center left", title = "Mass (kg)",bbox_to_anchor=(1, 0.5))
    PyPlot.savefig("./figures/changing_mass/$figname.png",transparent = true)
    ...
    figname = "pn_pd_norm"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1]/maximum(savestates[:,1]),-savestates[:,2]/maximum(-savestates[:,2]),".-",label = "$(round.(Mass,4))")
    ...
    figname = "t_T"
    PyPlot.figure(figname)
    PyPlot.plot(time_sim,savestates[:,end]./(P[:mass]*P[:gravity]),".-",label = "$(round.(Mass,4))")
    ...
    figname = "d_power"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1],power./(P[:mass]*P[:gravity]),".-",label = "$(round.(Mass,4))")
    # PyPlot.plot(time_sim,ones(time_sim)*P[:mass]*P[:gravity],"k-")
    PyPlot.xlabel("pn (m)")
    ...
    figname = "d_delta_e"
    PyPlot.figure(figname)
    PyPlot.plot(savestates[:,1],xopt[2:21],".-",label = "$(round.(Mass,4))")
    # PyPlot.plot(time_sim,ones(time_sim)*P[:mass]*P[:gravity],"k-")
    PyPlot.xlabel("pn (m)")
...
figname = "errorsum"
PyPlot.figure(figname)
PyPlot.plot(Mass_array,errorsum,".-",label = "$(round.(Mass_array,3))")
PyPlot.xlabel("Mass")
...
PyPlot.figure(figname)
PyPlot.plot(Mass_array,total_energy/(P[:mass]*P[:gravity]),".-",label = "$(round.(Mass_array,3))")
PyPlot.xlabel("Mass")
...
PyPlot.figure(figname)
PyPlot.plot(Mass_array,h_set./final_dist,".-",label = "$(round.(Mass_array,3))")
PyPlot.xlabel("Mass")
