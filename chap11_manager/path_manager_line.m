...
function out = path_manager_line(in,P,start_of_simulation)
  ...
  q = q/norm(q);
  n_plane = (q_old+q)/norm(q_old+q);
  planePierce = (p-waypoints(1:3,idx_waypoint))'*n_plane;
  if (p-waypoints(1:3,idx_waypoint))'*n_plane>=0 %&& idx_waypoint<num_waypoints-1
      idx_waypoint = idx_waypoint+1
  end
  
  out = [flag; Va_d; r; q_old; c; rho; lambda; state; flag_need_new_waypoints];
