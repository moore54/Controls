...
if ell<2*Rmin
    ...
    cr_start = ps+Rmin*[cos(chis+pi/2);sin(chis+pi/2);0];
    cl_start = ps+Rmin*[cos(chis-pi/2);sin(chis-pi/2);0];
    cr_end = pe+Rmin*[cos(chie+pi/2);sin(chie+pi/2);0];
    cl_end = pe+Rmin*[cos(chie-pi/2);sin(chie-pi/2);0];
    ...
    L1 = norm(cr_start-cr_end)+Rmin*(2*pi+(theta-pi/2)-(chis-pi/2))+Rmin*(2*pi+(chie-pi/2)-(theta-pi/2));
    % compute L2
    diffpspe = pe-ps;
    north = diffpspe(1);
    east = diffpspe(2);
    theta = atan2(east,north);
    ell = norm(cl_end-cr_start);
    theta2 = theta-pi/2+asin(2*Rmin/ell);
    if isreal(theta2)==0
        L2 = 9999;
    else
        L2 = sqrt((ell)^2-4*Rmin^2)+Rmin*(2*pi+(theta2)-(chis-pi/2))+Rmin*(2*pi+(theta2+pi)-(chie+pi/2));
    end
    % compute L3
    diffpspe = pe-ps;
    north = diffpspe(1);
    east = diffpspe(2);
    theta = atan2(east,north);
    ell = norm(cr_end-cl_start);
    theta2 = acos(2*Rmin/ell);
    if isreal(theta2)==0
        L3 = 9999;
    else
        L3 = sqrt((ell)^2-4*Rmin^2)+Rmin*(2*pi+(chis+pi/2)-(theta+theta2))+Rmin*(2*pi+(chie-pi/2)-(theta+theta2-pi));
    end
    % compute L4
    diffpspe = pe-ps;
    north = diffpspe(1);
    east = diffpspe(2);
    theta = atan2(east,north);
    L4 = norm(cl_start-cl_end)+Rmin*(2*pi+(chis+pi/2)-(theta+pi/2)) + Rmin*(2*pi+(theta+pi/2)-(chie+pi/2));
